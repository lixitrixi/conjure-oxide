...
/// - It returns a new subtree but the resulting [`Update`](crate::update::Update) is not chosen by the user-defined
/// selector function. The function may select a different rule's update or no update at all.
...
impl<T: Uniplate, M> Commands<T, M> {
    ...
    ///
    /// In this case, "pure" means that the transformation cannot register additional
    /// side-effects. The transformation function is given ownership of the tree and should return the
    /// updated tree.
    ///
